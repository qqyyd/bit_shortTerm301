...
QNChatMessage::QNChatMessage(QWidget *parent,QString friendName) : QWidget(parent)
{
    QFont te_font = this->font();
    te_font.setFamily("等线");
...
//            //自己的头
//            if (afriend.idNumber == myCommunicationClient::my_id)
//                myCommunicationClient::myinfo = afriend;
//            //对方的头
//            else if(afriend.idNumber == friendName)

//        }
//    });

    PersonalInfo HerInfo = myCommunicationClient::findInfo(friendName);

    setRightAvatar(myCommunicationClient::myinfo.image);
    setLeftAvatar(HerInfo.image);// 传入HerInfo.Image
    //m_rightPixmap = circleAvatar(QPixmap(":/Asserts/images/FishkoAvatar.png"));

    m_loadingMovie = new QMovie(this);
    m_loadingMovie->setFileName(":/Asserts/images/loading4.gif");
...
void QNChatMessage::setRightAvatar(QImage myAvatar)
{
    m_rightPixmap =  QPixmap::fromImage(myAvatar);
    circleAvatar(m_rightPixmap);
}
void QNChatMessage::setLeftAvatar(QImage friendAvatar)
{
    m_leftPixmap = QPixmap::fromImage(friendAvatar);
    circleAvatar(m_leftPixmap);
}
...
QPixmap QNChatMessage::circleAvatar(QPixmap pixmapa)
{
    int w = 80;
    int h = 80;
    QPixmap pixmap(w,h);
