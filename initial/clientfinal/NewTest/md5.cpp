...
public:
    ...
    string encode(string src) {
        QVector<unsigned int> rec = padding(src);
    ...
    // 填充字符串
    QVector<unsigned int> padding(string src) {
        // 以512位,64个字节为一组
        unsigned int num = ((src.length() + 8) / 64) + 1;
        QVector<unsigned int> rec(num*16);
        strlength = num*16;
        for(unsigned int i = 0; i < src.length(); i++){
            // 一个unsigned int对应4个字节，保存4个字符信息
//            rec[i>>2] |= ((string)((src[i]))).toInt() << ((i % 4) * 8);
            rec[i>>2] |= (int)(src[i]) << ((i % 4) * 8);
//            qDebug() << (int)(src[i]) << ' ' << (src[i]).digitValue() << endl;
        }
        // 补充1000...000
